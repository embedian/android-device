...
function get_current_device_base_name
{
    if [ -z ${current_device_base_name} ]; then
        ...
        for current_device_info in $all_device_info ; do
		    ...
		    current_device_base_name=$(echo ${current_device_base_name} | sed 's/p\?[0-9]\+$//')
                    IFS=$OLDIFS
                    return 0
                fi
            fi
...
function get_partition_node
{
    if [[ "${current_device_base_name}" == *"mmcblk"* ]] || \
	[[ "${current_device_base_name}" == *"nvme"* ]]; then
	   echo "${current_device_base_name}p$1"
    else
	   echo "${current_device_base_name}$1"
    fi
}
...
function format_partition
{
	...
	part_node=$(get_partition_node ${num})
	echo "format_partition: $1:${part_node} ${2:-ext4}"
        if [ "$2" != "f2fs" ]; then
	    mkfs.ext4 -F ${part_node} -L$1
        else
	    ...
	    simg2img /tmp/TemporaryFile_${randome_part} ${part_node}
...
function erase_partition
{
    ...
    if [ ${num} -gt 0 ] 2>/dev/null; then
        ...
        get_partition_size $1
	part_node=$(get_partition_node ${num})
	echo "erase_partition: $1 : ${part_node} ${g_sizes}M"
	dd if=/dev/zero of=${part_node} bs=1048576 conv=fsync,nocreat count=$g_sizes
    fi
}
...
function flash_partition
{
    ...
    do
        if [ $? -eq 0 ]; then
            ...
            get_current_device_base_name
	    part_node=$(get_partition_node ${num})
	    echo "flash_partition: ${img_name} ---> ${part_node}"

            if [ "$(echo ${1} | grep "vendor_boot")" != "" ]; then
	        dd if=${image_directory}${img_name} of=${part_node} bs=10M conv=fsync,nocreat
            elif [ "$(echo ${1} | grep "system")" != "" ] || [ "$(echo ${1} | grep "vendor")" != "" ] || \
                [ "$(echo ${1} | grep "product")" != "" ] || [ "$(echo ${1} | grep "super")" != "" ]; then
		simg2img ${image_directory}${img_name} ${part_node}
            else
		dd if=${image_directory}${img_name} of=${part_node} bs=10M conv=fsync,nocreat
            fi
        fi
