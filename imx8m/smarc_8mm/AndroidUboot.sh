...
build_imx_uboot()
{
	...
	cp ${UBOOT_OUT}/arch/arm/dts/imx8mm-smarc.dtb ${IMX_MKIMAGE_PATH}/imx-mkimage/iMX8M/
	...
	if [ `echo $2 | rev | cut -d '-' -f1 | rev` != "dual" ]; then
		cp ${UBOOT_OUT}/arch/arm/dts/imx8mm-smarc.dtb ${IMX_MKIMAGE_PATH}/imx-mkimage/iMX8M/
                make -C ${IMX_MKIMAGE_PATH}/imx-mkimage/ SOC=iMX8MM dtbs="${TARGET_DTBS}" flash_spl_uboot || exit 1
		cp ${UBOOT_OUT}/arch/arm/dts/imx8mm-smarc.dtb ${IMX_MKIMAGE_PATH}/imx-mkimage/iMX8M/
		 make -C ${IMX_MKIMAGE_PATH}/imx-mkimage/ SOC=iMX8MM dtbs="${TARGET_DTBS}" print_fit_hab || exit 1
	else
		cp ${UBOOT_OUT}/arch/arm/dts/imx8mm-smarc.dtb ${IMX_MKIMAGE_PATH}/imx-mkimage/iMX8M/
		make -C ${IMX_MKIMAGE_PATH}/imx-mkimage/ SOC=iMX8MM dtbs="${TARGET_DTBS}" flash_evk_no_hdmi_dual_bootloader || exit 1
		cp ${UBOOT_OUT}/arch/arm/dts/imx8mm-smarc.dtb ${IMX_MKIMAGE_PATH}/imx-mkimage/iMX8M/
		make -C ${IMX_MKIMAGE_PATH}/imx-mkimage/ SOC=iMX8MM dtbs="${TARGET_DTBS}" PRINT_FIT_HAB_OFFSET=0x0 print_fit_hab || exit 1
	fi
